  ...
  for (const batch of batches) {
    ...
    try {
      ...
      `, [
        ...
        availableQuantity,
        newQuantity,
        `Order delivery - ${product_name} (${deductedQuantity} ${unit})`
      ]);
      console.log(`Created inventory history entry for order delivery deduction`);
    } catch (historyError) {
      console.warn(`Failed to create inventory history entry:`, historyError);
    }
...
export async function registerRoutes(app: Express) {
  ...
  app.get("/api/purchase-history", isAuthenticated, async (req, res) => {
    try {
      ...
      console.log("Fetching all purchase history with params:", { pageNum, limitNum, search, supplierId, startDate, endDate });
  ...
  app.get("/api/inventory/history", isAuthenticated, async (req, res) => {
    try {
      ...
      // Parse limit and offset with proper defaults and validation
      ...
      console.log("Fetching inventory history with filters:", { inventoryId, productId, limit, offset });
      const history = await storage.getInventoryHistory(inventoryId, productId, limit, offset);
  ...
  // Debug endpoint to count total inventory history records
  app.get("/api/inventory/history/count", isAuthenticated, async (req, res) => {
    try {
      const conn = await storage.getConnection();
      try {
        const [countResult] = await conn.execute(`SELECT COUNT(*) as total FROM inventory_history`);
        const total = (countResult as any[])[0].total;
        console.log(`Total inventory history records in database: ${total}`);
        res.json({ total });
      } finally {
        conn.release();
      }
    } catch (error) {
      console.error("Count inventory history error:", error);
      res.status(500).json({ message: "Failed to count inventory history" });
    }
  });
  ...
  app.patch("/api/inventory/:id/quantity", isAuthenticated, async (req, res) => {
    try {
      ...
      try {
        ...
        // Create inventory history entry for quantity change
        try {
          const changeType = isAddition ? 'quantity_adjusted' : 'quantity_adjusted';
          const reason = isAddition
            ? `Inventory addition - ${quantity} units added`
            : `Caterer distribution - ${quantity} units sold`;
          ...
          console.log(`Created inventory history entry for quantity ${isAddition ? 'addition' : 'deduction'}`);
        } catch (historyError) {
          console.warn(`Failed to create inventory history entry:`, historyError);
        }
  ...
  app.patch("/api/inventory/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      try {
        ...
        // Get the current item before update for history tracking
        const [beforeRows] = await conn.execute(`SELECT * FROM inventory WHERE id = ?`, [id]);
        const beforeItem = (beforeRows as any[])[0];
        ...
        // Create inventory history entries for field changes
        if (beforeItem) {
          try {
            for (const field of fields) {
              ...
              if (oldValue !== newValue) {
                ...
                let reason = `Manual update - ${fieldDisplayName} changed`;

                // Handle special cases for field names and reasons
                if (field === 'quantity') {
                  reason = `Manual quantity adjustment - ${oldValue} → ${newValue}`;
                  await conn.execute(`
                    INSERT INTO inventory_history (
                      inventory_id, product_id, supplier_id, change_type,
                      field_changed, old_value, new_value, quantity_before, quantity_after,
                      reason, user_name, created_at
                    ) VALUES (?, ?, ?, 'quantity_adjusted', ?, ?, ?, ?, ?, ?, 'admin', NOW())
                  `, [
                    id, beforeItem.product_id, beforeItem.supplier_id,
                  ...
                  await conn.execute(`
                    INSERT INTO inventory_history (
                      inventory_id, product_id, supplier_id, change_type,
                      field_changed, old_value, new_value, reason, user_name, created_at
                    ) VALUES (?, ?, ?, 'updated', ?, ?, ?, ?, 'admin', NOW())
                  `, [
                    id, beforeItem.product_id, beforeItem.supplier_id,
                    field, String(oldValue), String(newValue), reason
                  ]);
                }

                console.log(`Created inventory history entry for ${field} change`);
              }
            }
          } catch (historyError) {
            console.warn(`Failed to create inventory history entries:`, historyError);
          }
        }
  ...
  app.get("/api/payment-reminders", isAuthenticated, async (_req, res) => {
    try {
      ...
      console.log("Cleaning up payment reminders before fetching...");
      await storage.cleanupPaidBillReminders();

      const reminders = await storage.getPaymentReminders();
      console.log(`Fetched ${reminders.length} payment reminders after cleanup`);

      // Additional server-side filtering to ensure no paid bills show up
      const filteredReminders = reminders.filter((reminder: any) => {
        // If reminder has distributionId, double-check it's not paid by querying the distribution
      ...
      console.log(`Returning ${filteredReminders.length} filtered payment reminders`);
  ...
  // Debug endpoint to see all payment reminders
  app.get("/api/payment-reminders/debug", isAuthenticated, async (_req, res) => {
    try {
      const conn = await storage.getConnection();
      try {
        const [reminders] = await conn.execute(`
          SELECT pr.*, d.status as dist_status, d.balance_due as dist_balance, d.bill_no
          FROM payment_reminders pr
          LEFT JOIN distributions d ON pr.distribution_id = d.id
        `);
        res.json(reminders);
      ...
      console.error("Debug payment reminders error:", error);
      res.status(500).json({ message: "Failed to debug payment reminders" });
    }
  });

  // Force cleanup endpoint - removes ALL payment reminders for testing
  app.post("/api/payment-reminders/force-cleanup", isAuthenticated, async (_req, res) => {
    try {
      console.log("FORCE cleanup of ALL payment reminders requested");
      const conn = await storage.getConnection();
      try {
        const [result] = await conn.execute(`DELETE FROM payment_reminders`);
        const deletedCount = (result as any).affectedRows;
        console.log(`Force deleted ${deletedCount} payment reminders`);
        res.json({ success: true, message: `Force deleted ${deletedCount} payment reminders` });
      } finally {
        conn.release();
      }
    } catch (error) {
  ...
  app.get("/api/customer-bills", isAuthenticated, async (req, res) => {
    try {
      ...
      console.log('Customer bills API params:', { page, limit, pageNum, limitNum, search, status });
          ...
          for (const [productIdStr, batchData] of Object.entries(selectedBatches)) {
            ...
            for (let i = 0; i < batchIds.length; i++) {
              ...
              if ((currentBatch as any[]).length > 0) {
                ...
                // Create inventory history entry for customer sale
                try {
                  console.log(`🔍 Creating inventory history entry for customer sale - Batch ID: ${batchId}, Product ID: ${productId}`);

                  // Get batch details for history entry
                  const [batchDetails] = await conn.execute(
                    `SELECT supplier_id FROM inventory WHERE id = ?`,
                    [batchId]
                  );

                  const supplierId = (batchDetails as any[])[0]?.supplier_id || null;
                  console.log(`📋 Batch details - Supplier ID: ${supplierId}`);
                  ...
                  const historyParams = [
                    ...
                    currentQuantity,
                    newQuantity,
                    `Customer sale - Bill #${billNo || 'Unknown'} - ${deductQuantity} units sold`
                  ];

                  console.log(`📝 Executing customer sale history INSERT with parameters:`, historyParams);
                  ...
                  console.log(`✅ Successfully created inventory history entry for customer sale`);
                } catch (historyError) {
                  console.error(`❌ Failed to create inventory history entry for customer sale:`, historyError);
                  console.error(`Error details:`, {
                    message: historyError.message,
                    code: historyError.code,
                    sqlState: historyError.sqlState
                  });
                }
  ...
  app.get("/api/caterers/:id/balance", isAuthenticated, async (req, res) => {
    try {
      ...
      console.log(`Found ${distributions.length} distributions and ${payments.length} payments`);

      // Method 1: Sum individual distribution balances (more accurate)
      const totalBilled = distributions.reduce((sum, dist) => {
        const amount = parseFloat(dist.grandTotal || '0');
        console.log(`Distribution ${dist.id} (${dist.billNo}): grandTotal=${amount}`);
        return sum + (isNaN(amount) ? 0 : amount);
      }, 0);

      const balanceDueFromDistributions = distributions.reduce((sum, dist) => {
        const balance = parseFloat(dist.balanceDue || '0');
        console.log(`Distribution ${dist.id} (${dist.billNo}): balanceDue=${balance}`);
        return sum + (isNaN(balance) ? 0 : balance);
      }, 0);

      // Method 2: Total payments approach (for verification)
      const totalPaid = payments.reduce((sum, payment) => {
        const amount = parseFloat(payment.amount || '0');
        console.log(`Payment ${payment.id}: amount=${amount}, distributionId=${payment.distributionId}`);
        return sum + (isNaN(amount) ? 0 : amount);
      }, 0);

      const balanceDueFromTotals = Math.max(0, totalBilled - totalPaid);

      console.log(`Calculation results:`);
      console.log(`- Total Billed: ${totalBilled}`);
      console.log(`- Total Paid: ${totalPaid}`);
      console.log(`- Balance from distributions: ${balanceDueFromDistributions}`);
      console.log(`- Balance from totals: ${balanceDueFromTotals}`);

      // Use the distribution-based calculation as it's more accurate for individual bill tracking
      const balanceDue = balanceDueFromDistributions;
  ...
  app.post("/api/caterers/:id/sync-balance", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ error: 'Invalid caterer ID' });
      ...
      console.log(`Sync balance for caterer ${id}:`);
      console.log(`Found ${distributions.length} distributions and ${payments.length} payments`);
      ...
      console.log(`Sync calculation results:`);
      console.log(`- Total Billed: ${totalBilled}`);
      console.log(`- Total Paid: ${totalPaid}`);
  ...
  // Debug endpoint to check distribution and payment data
  app.get("/api/distributions/:id/debug", isAuthenticated, async (req, res) => {
    try {
      const distributionId = parseInt(req.params.id);
      if (isNaN(distributionId)) {
        return res.status(400).json({ error: 'Invalid distribution ID' });
      ...
      const conn = await storage.getConnection();
      try {
        const [distRows] = await conn.execute('SELECT * FROM distributions WHERE id = ?', [distributionId]);
        const [paymentRows] = await conn.execute('SELECT * FROM caterer_payments WHERE distribution_id = ?', [distributionId]);

        res.json({
          distributionId,
          distribution: {
            raw: (distRows as any[])[0],
            mapped: distribution
          },
          payments: {
            raw: paymentRows,
          ...
          calculations: {
            grandTotal: parseFloat(distribution.grandTotal || '0'),
            currentAmountPaid: parseFloat(distribution.amountPaid || '0'),
            currentBalanceDue: parseFloat(distribution.balanceDue || '0'),
            calculatedTotalPaid: payments.reduce((sum, p) => sum + parseFloat(p.amount || '0'), 0),
            calculatedBalanceDue: Math.max(0, parseFloat(distribution.grandTotal || '0') - payments.reduce((sum, p) => sum + parseFloat(p.amount || '0'), 0))
          }
        });
      } finally {
        conn.release();
      ...
      res.status(500).json({ error: 'Failed to debug distribution' });
    }
  });

  // Test endpoint to check distribution balance calculation
  app.get("/api/distributions/:id/balance-check", isAuthenticated, async (req, res) => {
    try {
      const distributionId = parseInt(req.params.id);
      if (isNaN(distributionId)) {
        return res.status(400).json({ error: 'Invalid distribution ID' });
      ...
      const grandTotal = parseFloat(distribution.grandTotal || '0');
      const currentAmountPaid = parseFloat(distribution.amountPaid || '0');
      const currentBalanceDue = parseFloat(distribution.balanceDue || '0');
      ...
      const calculatedBalanceDue = Math.max(0, grandTotal - calculatedTotalPaid);
      const calculatedStatus = calculatedBalanceDue <= 0 ? 'paid' : (calculatedTotalPaid > 0 ? 'partial' : 'active');

      res.json({
        distributionId,
        billNo: distribution.billNo,
        grandTotal,
        current: {
        ...
        calculated: {
          amountPaid: calculatedTotalPaid,
        ...
        isCorrect: Math.abs(currentAmountPaid - calculatedTotalPaid) < 0.01 &&
                   Math.abs(currentBalanceDue - calculatedBalanceDue) < 0.01 &&
                   distribution.status === calculatedStatus
      });
    } catch (error) {
      console.error('Error checking distribution balance:', error);
      res.status(500).json({ error: 'Failed to check distribution balance' });
    }
  });

  // Fix balance for a specific distribution
  app.post("/api/distributions/:id/fix-balance", isAuthenticated, async (req, res) => {
    try {
      const distributionId = parseInt(req.params.id);
      if (isNaN(distributionId)) {
        return res.status(400).json({ error: 'Invalid distribution ID' });
      ...
      try {
        await conn.execute(`
          ...
          WHERE id = ?
        `, [calculatedTotalPaid, calculatedBalanceDue, calculatedStatus, distributionId]);
        ...
        res.json({
          ...
          billNo: distribution.billNo,
          before: {
            amountPaid: parseFloat(distribution.amountPaid || '0'),
            balanceDue: parseFloat(distribution.balanceDue || '0'),
            status: distribution.status
          },
          after: {
            amountPaid: calculatedTotalPaid,
      ...
      console.error('Error fixing distribution balance:', error);
      res.status(500).json({ error: 'Failed to fix distribution balance' });
    }
  });

  // Run the distribution balance fix migration manually
  app.post("/api/admin/fix-distribution-balances", isAuthenticated, async (req, res) => {
    try {
      console.log('🔧 Starting distribution balance fix migration...');

      const conn = await storage.getConnection();
      try {
        // Step 1: Create UpdateDistributionBalance procedure
        await conn.execute(`DROP PROCEDURE IF EXISTS UpdateDistributionBalance`);
        await conn.execute(`
        ...
        const [distributions] = await conn.execute(`SELECT id FROM distributions`);
        const distIds = (distributions as any[]).map(d => d.id);

        console.log(`🔄 Fixing balances for ${distIds.length} distributions...`);

        for (const distId of distIds) {
          await conn.execute(`CALL UpdateDistributionBalance(?)`, [distId]);
        }

        console.log('✅ Fixed all distribution balances');

        // Step 3: Create triggers for automatic balance updates
        await conn.execute(`DROP TRIGGER IF EXISTS update_distribution_balance_after_payment_insert`);
        await conn.execute(`
        ...
        await conn.execute(`DROP TRIGGER IF EXISTS update_distribution_balance_after_payment_update`);
        await conn.execute(`
        ...
        await conn.execute(`DROP TRIGGER IF EXISTS update_distribution_balance_after_payment_delete`);
        await conn.execute(`
        ...
        const [emiExamples] = await conn.execute(`
          SELECT
            d.id,
            d.bill_no,
            d.grand_total,
        ...
        res.json({
          success: true,
          message: 'Distribution balance fix completed successfully!',
          results: {
            totalDistributions: result.total_distributions,
            correctBalances: result.correct_balances,
            emiExamples: emiExamples
          }
        });
      ...
      res.status(500).json({
        success: false,
        error: 'Failed to fix distribution balances',
        details: error.message
      });
    }
  });

  // Fix distribution balances by recalculating from payments
  app.post("/api/distributions/fix-balances", isAuthenticated, async (req, res) => {
    try {
      const distributions = await storage.getDistributions();
      const results = [];
      ...
      for (const distribution of distributions) {
        try {
          // Get all payments for this distribution
          const payments = await storage.getCatererPaymentsByDistribution(distribution.id);
          ...
          const correctBalanceDue = Math.max(0, grandTotal - totalPaid);
          const correctStatus = correctBalanceDue <= 0 ? 'paid' : (totalPaid > 0 ? 'partial' : 'active');
          ...
          if (Math.abs(currentAmountPaid - totalPaid) > 0.01 ||
              Math.abs(currentBalanceDue - correctBalanceDue) > 0.01 ||
              distribution.status !== correctStatus) {

            // Update the distribution
            await storage.updateDistribution(distribution.id, {
              amountPaid: totalPaid.toString(),
              balanceDue: correctBalanceDue.toString(),
            ...
            results.push({
              ...
              changes: {
                amountPaid: { old: currentAmountPaid, new: totalPaid },
                balanceDue: { old: currentBalanceDue, new: correctBalanceDue },
                status: { old: distribution.status, new: correctStatus }
              }
            });
          } else {
            results.push({
      ...
      res.json({
        success: true,
        message: `Fixed ${fixedCount} out of ${totalCount} distributions`,
        results: results
      });
    } catch (error) {
      console.error('Error fixing distribution balances:', error);
      res.status(500).json({ error: 'Failed to fix distribution balances' });
    }
  });

  // Sync all caterer balances (fix discrepancies for all caterers)
  app.post("/api/caterers/sync-all-balances", isAuthenticated, async (req, res) => {
    try {
      const caterers = await storage.getCaterers();
      const results = [];
